...
const main = async () => {
	...
	if (getArgFlag('-h') || args.length != 1) {
		console.log('Usage: updateMaxMintPerWallet.js XX');
		console.log('   where XX is the max mints per wallet, 0 == unbounded');
		return;
	}
	...
	console.log('Max per wallet:\n', mintEconomics[7]);

	const maxPerWallet = Number(args[0]);
	console.log('New max:', maxPerWallet);
	const proceed = readlineSync.keyInYNStrict('Do you want to update max per wallet?');
	if (proceed) {
		await useSetterInts('updateMaxMintPerWallet', maxPerWallet);
		mintEconomics = await getSetting('getMintEconomics', 'mintEconomics');
		console.log('New Max per wallet:\n', mintEconomics[7]);
	}
	else {
