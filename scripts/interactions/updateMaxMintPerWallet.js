...
const main = async () => {
	...
	if (getArgFlag('-h') || args.length != 1) {
		console.log('Usage: updateMaxMintPerWallet.js X');
		console.log('X - number of mints allowed per wallet');
		return;
	}
	...
	const settting = Number(args[0]);
	const msg = `Update max mint per wallet to ${args[0]}?`;
