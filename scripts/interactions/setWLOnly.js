...
const main = async () => {
	...
	// get the current WL
	const encodedWL = mintIface.encodeFunctionData('getWhitelist');
	const whitelistOutput = await readOnlyEVMFromMirrorNode(
		env,
		contractId,
		encodedWL,
		operatorId,
	...
	const whitelist = mintIface.decodeFunctionResult('getWhitelist', whitelistOutput)[0];
