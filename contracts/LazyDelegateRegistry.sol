...
contract LazyDelegateRegistry is ILazyDelegateRegistry {
    ...
    function delegateNFT(
        ...
        bytes32 delegateTokenHash = keccak256(
            abi.encodePacked(_delegate, _token)
        );
        bytes32 ownerTokenHash = keccak256(
            abi.encodePacked(msg.sender, _token)
        );
        ...
        for (uint256 i = 0; i < serialLength; ) {
            ...
            bool removed = _revokeDelegateNFT(
                ownerTokenHash,
                currentOwner,
                _token,
                _serial,
            ...
            if (!removed) {
                ...
                bytes32 tokenSerialHash = keccak256(
                    abi.encodePacked(_token, _serial)
                );
     ...
     */
    function _revokeDelegateNFT(
        bytes32 _ownerTokenHash,
        address _currentOwner,
        address _token,
        uint256 _serial,
        bool _fullRemoval
    ) internal returns (bool _removed) {
        // get current delegate
        (bool found, address currentDelegate) = delegatedNFT[_token].tryGet(
            _serial
        );
        ...
        if (found && currentDelegate != address(0)) {
            ...
            address currentDelegator = delegatedNFTSerialsOwnerByHash[
                tokenSerialHash
            ];
            bytes32 currentDelegatorTokenHash = keccak256(
                abi.encodePacked(currentDelegator, _token)
            );
            ...
            bytes32 delegateTokenHash = keccak256(
                abi.encodePacked(currentDelegate, _token)
            );
            ...
            if (_fullRemoval || currentDelegator != _currentOwner) {
                delegatedNFTSerialsByHash[currentDelegatorTokenHash].remove(
                    _serial
                );
                if (
                    delegatedNFTSerialsByHash[currentDelegatorTokenHash]
                        .length() == 0
                ) {
                    // tidy up the address set
                    walletToTokenDelegations[currentDelegator].remove(_token);
    ...
    function checkNFTDelegationIsValidBatch(
        ...
        for (uint256 i = 0; i < _tokens.length; i++) {
            ...
            for (uint256 j = 0; j < _serials[i].length; j++) {
                valid[i][j] = checkNFTDelegationIsValid(
                    _tokens[i],
                    _serials[i][j]
                );
            }
        ...
        returns (address[] memory tokens, uint256[][] memory serials)
    {
        ...
        for (uint256 i = 0; i < tokens.length; i++) {
            bytes32 delegateTokenHash = keccak256(
                abi.encodePacked(_delegate, tokens[i])
            );
        ...
        returns (address[] memory tokens, uint256[][] memory serials)
    {
        ...
        if (_includeSerials) {
            ...
            for (uint256 i = 0; i < tokens.length; i++) {
                bytes32 ownerTokenHash = keccak256(
                    abi.encodePacked(_ownerWallet, tokens[i])
                );
    ...
    function getSerialsDelegatedToRange(
        ...
        uint256[] memory delegateTokenSerials = delegatedNFTSerialsByHash[
            keccak256(abi.encodePacked(_delegate, _token))
        ].values();
        require(
            _offset + _limit <= delegateTokenSerials.length,
            "LDR: Range OOB"
        );
    ...
    function getSerialsDelegatedBy(
        ...
        bytes32 ownerDelegateHash = keccak256(
            abi.encodePacked(_ownerWallet, _token)
        );
    ...
    function getSerialsDelegatedByRange(
        ...
        serials = new uint256[](_limit);
        // get the serial list for the delegate and token
        uint256[] memory ownerDelegateSerials = delegatedNFTSerialsByHash[
            keccak256(abi.encodePacked(_ownerWallet, _token))
        ].values();
        require(
            _offset + _limit <= ownerDelegateSerials.length,
            "LDR: Range OOB"
        );
        // fill serials array based on offset and limit
    ...
    function getTokensWithDelegatesRange(
        ...
        require(
            _offset + _limit <= tokensWithDelegates.length(),
            "LDR: Range OOB"
        );
    ...
    function getWalletsWithDelegatesRange(
        ...
        require(
            _offset + _limit <= walletsWithDelegates.length(),
            "LDR: Range OOB"
        );
