    ...
    ReentrancyGuard
{
    ...
    event GasStationStatus(string message, address sender, uint256 value);
    ...
    constructor(address _lazyToken, address _lazySCT) {
    ...
    modifier onlyAuthorizer() {
        if (!authorizers.contains(msg.sender))
            revert PermissionDenied(msg.sender, Role.GasStationAuthorizer);
        _;
    }

    modifier onlyContractUser() {
        if (!contractUsers.contains(msg.sender))
            revert PermissionDenied(msg.sender, Role.GasStationContractUser);
        _;
    }
    ...
    function payoutLazy(
        ...
        if (burnAmt > 0) {
            ...
            if (remainder > 0) {
                result = IERC20(lazyToken).transfer(_user, remainder);
                if (!result) {
            ...
            result = IERC20(lazyToken).transfer(_user, _amount);
            if (!result) {
                revert PayoutFailed();
            }
    ...
    function drawLazyFromPayTo(
        ...
        if (IERC20(lazyToken).allowance(_user, address(this)) < _amount) {
            ...
            _amount == 0 || _burnPercentage > 100 || _payTo == address(0)
        ) {
            revert BadInput();
        }
        ...
        if (burnAmt > 0) {
            ...
            result = IERC20(lazyToken).transferFrom(_user, _payTo, _amount);
            if (!result) {
                revert PayoutFailed();
            }
        }
    ...
    function addAdmin(address _admin) external onlyAdmin returns (bool _added) {
    ...
    function removeAdmin(
        ...
        emit GasStationAccessControlEvent(
            msg.sender,
            _admin,
            false,
            Role.Admin
    ...
    function getAuthorizers()
        external
        view
        returns (address[] memory _authorizers)
    {
    ...
    function getContractUsers()
        external
        view
        returns (address[] memory _contractUsers)
    {
    ...
    function isAuthorizer(
        address _authorizer
    ) external view returns (bool _isAuthorizer) {
    ...
    ) external view returns (bool _isContractUser) {
    ...
    receive() external payable {
        emit GasStationStatus("Receive", msg.sender, msg.value);
    }

    fallback() external payable {
        emit GasStationStatus("Fallback", msg.sender, msg.value);
    }
}
